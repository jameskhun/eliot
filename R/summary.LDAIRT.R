summary.LDAIRT <- function(LDAIRT.out,print.topwords=TRUE,print.topvotes=TRUE,print.ordering=FALSE){
	...
	for (k in 1:ntopics){
		...
		if (print.topwords) print(paste("Distinctive Words: ",paste(names(sort(wordtopicevidence[k,],decreasing=TRUE))[1:10],collapse=", "),sep=""))
		TopVotes[,k] <- sort(lambda.est[k,],decreasing=TRUE,index.return=TRUE)$ix[1:20]
		if (print.topvotes) print(paste("Most Topical Votes: ",paste(paste(colnames(lambda.est)[sort(lambda.est[k,],decreasing=TRUE,index.return=TRUE)$ix],paste("(",round(lambda.est[k,],2),")",sep="")[sort(lambda.est[k,],decreasing=TRUE,index.return=TRUE)$ix])[1:10],collapse="; "),sep=""))
		if (print.ordering) print(paste("Voter Ordering: ",paste(names(sort(rank.est[k,],decreasing=FALSE)),collapse=", "),sep=""))
